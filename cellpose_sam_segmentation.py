...
def save_pixel_to_cell_csv(masks, output_dir, image_name):
    """
    ...
    # Mark interior (non-boundary) pixels as potential nucleus regions
    ...
    logger.info(f"  Interior (nucleus) pixels: {df_full['is_interior'].sum():,}")
    logger.info(f"  Boundary (membrane) pixels: {df_full['is_boundary'].sum():,}")
    logger.info(f"  Background pixels: {(df_full['cell_id'] == 0).sum():,}")

    # Cell-level summary is optional - you can compute it from the full CSV if needed
...
def main():
    ...
    output_dir = current_dir / 'cellpose_sam_output'
    ...
    logger.info("="*70)
    full_csv_path, summary_csv_path = save_pixel_to_cell_csv(masks, output_dir, image_name)
    ...
    logger.info(f"  - Masks (TIFF): {masks_tif_path}")
    ...
    logger.info("    - is_boundary: 1 if pixel is at cell boundary, 0 otherwise")
