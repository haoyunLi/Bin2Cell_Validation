...
def save_nuclear_assignments(adata, output_dir, dataset_name):
    ...
    return assignments, cell_summary
...
def save_results(adata, cdata, output_dir, dataset_name):
    ...
    save_nuclear_assignments(adata, output_dir, dataset_name)
...
def main():
    ...
    # Generate scaled H&E image and perform segmentation
    ...
    # Save results
    save_results(adata, cdata, paths['output_dir'], paths['dataset_name'])

    logger.info("="*80)
    logger.info("Bin2cell analysis complete!")
    logger.info("="*80)
    logger.info(f"Results saved to: {paths['output_dir']}")
    logger.info(f"  Bin-level data: {paths['dataset_name']}_bin2cell_2um.h5ad")
    logger.info(f"  Cell-level data: {paths['dataset_name']}_bin2cell_cells.h5ad")
